...
CREATE TABLE `rbac_group` (
  ...
  KEY `group_parentid` (`group_parentid`),
  CONSTRAINT `rbac_group_ibfk_1` FOREIGN KEY (`group_parentid`) REFERENCES `rbac_group` (`group_id`) ON DELETE NO ACTION ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `rbac_menu` (
  ...
  `menu_type` int(3) default NULL,
  PRIMARY KEY  (`menu_id`),
  KEY `menu_parentid` (`menu_parentid`),
  CONSTRAINT `rbac_menu_ibfk_1` FOREIGN KEY (`menu_parentid`) REFERENCES `rbac_menu` (`menu_id`) ON DELETE NO ACTION ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
