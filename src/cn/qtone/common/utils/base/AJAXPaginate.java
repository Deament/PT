...
public class AJAXPaginate
{
	...
	{
		setUrl(request.getRequestURL().toString() + "?" + request.getQueryString(), funcName);
	}
	 ...
	 * @param funcName
	 */
	public void setUrl(HttpServletRequest request, String paras, 
			String funcName)
	{
	...
	public void setUrl(HttpServletRequest request)
	{
		...
		String param = StringUtils.trimToEmpty(request.getQueryString()).intern();
	...
	protected void parse(String url, String funcName)
	{
		...
		String paras = url + (url.indexOf('?') == -1 ? "?" : "&")
					+ "page=";
